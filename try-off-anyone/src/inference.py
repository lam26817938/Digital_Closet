...
def test_image(myurl=''):
    ...
    image = Image.open(get_image_file(myurl))
    ...
    for filename in os.listdir(directory):
        if filename.startswith('clo') and filename.endswith('.png'):
            # 提取檔案名稱中的數字部分
            try:
                number = int(filename[len('clo'):-len('.png')])
                max_number = max(max_number, number)
            except ValueError:
                pass  # 略過非數字命名的檔案

    filename = f"clo{max_number + 1}.png"
    laydown_image.save(os.path.join(directory, filename))
